...
Output::Output()
{
	...
	UI.PlayerColors[0] = TOMATO;
	UI.PlayerColors[1] = GREEN;
	UI.PlayerColors[2] = BLUE;
	UI.PlayerColors[3] = MAGENTA;
...
{
	return  UI.wx + cellPos.HCell() * UI.CellWidth;
...
{
	return  UI.wy + cellPos.VCell() * UI.CellHeight;
}
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_DELETE_GAMEOBJECT] = "images\\delete.jpg";
	MenuItemImages[ITM_SAVED_GRID] = "images\\save.jpg";
	MenuItemImages[ITM_OPEN_GRID] = "images\\folder_open.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_EXIT2] = "images\\Menu_Exit2.jpg";
	///TODO: Prepare images for each menu item and add it to the list
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	if (fromCell.VCell() == toCell.VCell() && fromCell.HCell() == toCell.HCell()) return;
	if ((fromCell.HCell() == 10 || toCell.HCell() == 10) && toCell.VCell() == 0) return; //this condition to not draw a ladder at cellNum 99
	if ((fromCell.HCell() == 0 || toCell.HCell() == 0) && fromCell.VCell() == 8) return;
	if (fromCell.HCell() != toCell.HCell() || fromCell.VCell() - toCell.VCell() < 0) return;
	...
	for (int i = y1 - UI.CellHeight / 2; i > y2; i -= UI.CellHeight)
		pWind->DrawLine(x12, i, x34, i);
}

//////////////////////////////////////////////////////////////////////////////////////////

void Output::DrawSnake(const CellPosition& toCell, const CellPosition& fromCell) const
{

	///TODO: Validate the fromCell and toCell (Must be Vertical and toCell is below fromCell otherwise do NOT draw)

	
	if ((fromCell.HCell() == 10 || toCell.HCell() == 10) && toCell.VCell() == 0) return;
	if ((fromCell.HCell() == 0 || toCell.HCell() == 0) && fromCell.VCell() == 8) return;
	if (toCell.VCell() == fromCell.VCell()) return;
	if (fromCell.VCell() - toCell.VCell() < 0) return;

	// Get the upper left corner coordinates of the fromCell and toCell
	int cellStartX = GetCellStartX(toCell); // same for fromCell and toCell (vertical)
	...
	pWind->DrawPolygon(px, py, 4, FILLED);
	pWind->DrawLine(x12, y1 - yChange, x12, y1 - yChange - yChange / 2);
}
