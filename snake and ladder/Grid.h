...
public:
	...
	int AddObjectToCell(GameObject * pNewObject);		 // Adds a GameObject to the Cell of its "position" data member
	...
	void UpdateInterface() const;		// It Updeats the Grid according to the last state of the game
	...
	bool IntersectWithSnake (GameObject* newObj) const;
	bool IntersectWithLadder(GameObject* newObj) const;
	void deleteALL();
	void SaveAll(ofstream& OutFile, Type obj);
